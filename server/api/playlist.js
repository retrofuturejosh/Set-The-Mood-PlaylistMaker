                ...
                tags.forEach(tag => {
                  ...
                  if (tag != req.query.artist && tag != req.query.track) {
                    tagsArr.forEach(moreTag => {
                      let secondarySimilarity = stringSimilarity.compareTwoStrings(
                        tag.name,
                        moreTag
                      );
    ...
    .then(trackOptions => {
      ...
      for (let i = 0; i < 250; i++) {
        ...
        songCheck.forEach(song => {
          let similarSongCheck = stringSimilarity.compareTwoStrings(
            song,
            checkSong
          );
      ...
      let youTubePromise = finalMatches.map(finalSong => {
        return new Promise((resolve, reject) => {
          youTubeSearch(
            `${finalSong.name} ${finalSong.artist} official`,
            youTubeOpts,
